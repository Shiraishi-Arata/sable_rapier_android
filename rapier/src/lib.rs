...
pub(crate) static SCENES: OnceLock<RwLock<HashMap<i32, Arc<scene::PhysicsScene>>>> =
    OnceLock::new();

#[macro_export]
macro_rules! jni_scene_fn {
    ($name:ident; $env:pat, ($($extra:tt)*), $scene:ident, $body:block) => {
        paste::paste! {
            #[unsafe(no_mangle)]
...
#[macro_export]
macro_rules! jni_scene_fn_ret {
    ($name:ident; $env:pat, ($($extra:tt)*), -> $ret:ty, $scene:ident, $body:block) => {
        paste::paste! {
            #[unsafe(no_mangle)]
            pub extern "system" fn $name<'local>(
                $env: ::jni::JNIEnv<'local>,
...
fn initialize_impl<'local>(
    env: &mut JNIEnv<'local>,
    x: jdouble,
    y: jdouble,
    z: jdouble,
    universal_drag: jdouble,
) -> Arc<scene::PhysicsScene> {
    ...
    {
        let mut sim_data = s.sim_data.write().unwrap();
        sim_data.collider_set.insert(collider);

        s.ground_handle = Some(sim_data.rigid_body_set.insert(ground));
    }
...
pub extern "system" fn Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_initialize_v1<'local>(
    ...
    let scene = initialize_impl(&mut env, x, y, z, universal_drag);
    let scenes = SCENES.get_or_init(|| RwLock::new(HashMap::new()));
    scenes.write().unwrap().insert(scene_id as i32, scene);
}
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_tick; mut env, (_time_step: jdouble), scene, {
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_step; mut env, (time_step: jdouble), scene, {
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_getPose; env, (id: jint, store: JDoubleArray<'local>), scene, {
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_setCenterOfMass; _env, (id: jint, x: jdouble, y: jdouble, z: jdouble), scene, {
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_setLocalBounds; _env, (id: jint, min_x: jint, min_y: jint, min_z: jint, max_x: jint, max_y: jint, max_z: jint), scene, {
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_createSubLevel; env, (id: jint, pose: JDoubleArray<'local>), scene, {
        ...
        let rb_handle = sim_data.rigid_body_set.insert(rigid_body);
        ...
        sable_data
            .rigid_bodies
            .insert(id as LevelColliderID, rb_handle);
    }
}

jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_removeSubLevel; _env, (id: jint), scene, {
        let mut sable_data = scene.sable_data.write().unwrap();

        sable_data.level_colliders.remove(&(id as LevelColliderID));
        let rb_handle = sable_data
            .rigid_bodies
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_addChunk; env, (x: jint, y: jint, z: jint, data: JIntArray<'local>, global: jboolean, object_id: jint), scene, {
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_removeChunk; _env, (x: jint, y: jint, z: jint, global: jboolean), scene, {
        let physics_state = get_physics_state();
        let collider_map = &physics_state.voxel_collider_map;
        let mut sable_data = scene.sable_data.write().unwrap();
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_changeBlock; _env, (x: jint, y: jint, z: jint, block: jint), scene, {
        let block_collider_id = (block >> 16) as u16;
        let voxel_state_id = (block & 0xFFFF) as u16;
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_setMassProperties; env, (id: jint, mass: jdouble, center_of_mass: JDoubleArray<'local>, inertia: JDoubleArray<'local>), scene, {
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_teleportObject; _env, (id: jint, x: jdouble, y: jdouble, z: jdouble, i: jdouble, j: jdouble, k: jdouble, r: jdouble), scene, {
        let sable_data = scene.sable_data.read().unwrap();
        let mut sim_data = scene.sim_data.write().unwrap();
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_wakeUpObject; _env, (id: jint), scene, {
        let sable_data = scene.sable_data.read().unwrap();
        let mut sim_data = scene.sim_data.write().unwrap();
        let rb = &mut sim_data.rigid_body_set[sable_data.rigid_bodies[&(id as LevelColliderID)]];
        rb.wake_up(true);
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_addLinearAngularVelocities; _env, (id: jint, linear_x: jdouble, linear_y: jdouble, linear_z: jdouble, angular_x: jdouble, angular_y: jdouble, angular_z: jdouble, wake_up: jboolean), scene, {
        let sable_data = scene.sable_data.read().unwrap();
        let mut sim_data = scene.sim_data.write().unwrap();
...
jni_scene_fn_ret! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_clearCollisions; _env, (), -> JDoubleArray<'local>, scene, {
        let arr: Vec<jdouble> = {
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_applyForce; _env, (id: jint, x: jdouble, y: jdouble, z: jdouble, fx: jdouble, fy: jdouble, fz: jdouble, wake_up: jboolean), scene, {
        let sable_data = scene.sable_data.read().unwrap();
        let mut sim_data = scene.sim_data.write().unwrap();
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_applyForceAndTorque; _env, (id: jint, fx: jdouble, fy: jdouble, fz: jdouble, tx: jdouble, ty: jdouble, tz: jdouble, wake_up: jboolean), scene, {
        let sable_data = scene.sable_data.read().unwrap();
        let mut sim_data = scene.sim_data.write().unwrap();
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_getLinearVelocity; _env, (id: jint, store: JDoubleArray<'local>), scene, {
        let sable_data = scene.sable_data.read().unwrap();
        let sim_data = scene.sim_data.read().unwrap();
...
jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_getAngularVelocity; _env, (id: jint, store: JDoubleArray<'local>), scene, {
        let sable_data = scene.sable_data.read().unwrap();
        let sim_data = scene.sim_data.read().unwrap();
...
macro_rules! v1nm {
    ($name:expr, $sig:expr, $fn:path) => {
        ::jni::NativeMethod {
            name: $name.into(),
            sig: $sig.into(),
            fn_ptr: $fn as *mut std::ffi::c_void,
        }
    };
}
...
pub unsafe extern "system" fn JNI_OnLoad(
    ...
    let class = match env.find_class("dev/ryanhcode/sable/physics/impl/rapier/Rapier3D") {
        Ok(c) => c,
        Err(_) => return ::jni::sys::JNI_VERSION_1_6,
    };

    let is_v2 = env
        .get_static_method_id(&class, "initialize", "(DDDD)J")
        .is_ok();
