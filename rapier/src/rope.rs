...
crate::jni_scene_fn_ret! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_createRope; env, (point_radius: jdouble, first_joint_length: jdouble, points: JDoubleArray<'local>, num_points: jint), -> jlong, scene, {
...
crate::jni_scene_fn_ret! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_queryRope; env, (id: jlong), -> JDoubleArray<'local>, scene, {
...
crate::jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_setRopeFirstSegmentLength; _env, (id: jlong, length: jdouble), scene, {
        let mut sable_data = scene.sable_data.write().unwrap();
        let mut sim_data = scene.sim_data.write().unwrap();
...
crate::jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_removeRopePointAtStart; _env, (id: jlong), scene, {
        let mut sable_data = scene.sable_data.write().unwrap();
        let mut sim_data = scene.sim_data.write().unwrap();
        let sim_data = &mut *sim_data;
...
crate::jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_addRopePointAtStart; _env, (id: jlong, x: jdouble, y: jdouble, z: jdouble), scene, {
        let mut sim_data = scene.sim_data.write().unwrap();
        let mut sable_data = scene.sable_data.write().unwrap();
        let universal_drag = scene.universal_drag;
...
crate::jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_wakeUpRope; _env, (rope_id: jlong), scene, {
        let sable_data = scene.sable_data.read().unwrap();
        let mut sim_data = scene.sim_data.write().unwrap();
...
crate::jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_setRopeAttachment; _env, (rope_id: jlong, sub_level_id: jint, x: jdouble, y: jdouble, z: jdouble, end: jboolean), scene, {
