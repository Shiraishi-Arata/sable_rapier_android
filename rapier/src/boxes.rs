...
crate::jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_createBox; env, (id: jint, mass: jdouble, half_extent_x: jdouble, half_extent_y: jdouble, half_extent_z: jdouble, pose: JDoubleArray<'local>), scene, {
...
crate::jni_scene_fn! {
    Java_dev_ryanhcode_sable_physics_impl_rapier_Rapier3D_removeBox; _env, (id: jint), scene, {
        let mut sim_data = scene.sim_data.write().unwrap();
        let sim_data = &mut *sim_data;
        let mut sable_data = scene.sable_data.write().unwrap();
